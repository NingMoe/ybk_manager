...
namespace YbkManage.Activities
{
    ...
    public class ReportListByTeacher : AppActivity, SwipeRefreshLayout.IOnRefreshListener, IRecyclerViewItemClickListener, View.IOnClickListener
    {
        ...
        {
            FindViewById<TextView>(Resource.Id.tv_title).Text = string.Format("{0}续班情况", currReportInfo.Item3);
        ...
        /// 获取报表数据
        /// </summary>
        private void GetRenewInfoInClassByTeacher()
        {
			try
			{
				new Thread(new ThreadStart(() =>
				{
					...
					var result = RenewService.GetRenewInfoInClassByTeacher(CurrUserInfo.SchoolId, searchQuarter.Year, searchQuarter.Quarter, gradeStr, districtStr, currReportInfo.Item2, 1, 0, classStatus);
					RunOnUiThread(() =>
					{
						...
						if (result != null)
						{
							renewInfoList = result.RenewInfo;
							mAdapter.SetData(renewInfoList);
							mAdapter.NotifyDataSetChanged();
						}
					});
				})).Start();
