...
namespace xxxxxLibrary.Network
{
    public class HttpRequestUtil
    {
		...
		public static async Task<object> SendPostRequestBasedOnHttpClient(string requestUrl, IDictionary<string, string> routeParameters = null)
		{
		...
		/// <param name="routeParameters">the parameters you post</param>
		/// <returns>return a response object</returns>
        public static async Task<object> SendGetRequestBasedOnHttpClient(string requestUrl, IDictionary<string, string> routeParameters = null)
		{
			object returnValue = new object();
			HttpClient client = new HttpClient();
			client.MaxResponseContentBufferSize = 256000;
            ...
            {
				paramters = string.Join("&", routeParameters.Select(p => p.Key + "=" + p.Value));
            }
